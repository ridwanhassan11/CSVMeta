...
export default function ControlsPanel({
  ...
  const isFreepik = settings.platform === "freepik";

  function handlePlatformClick(id: Platform) {
    if (id === "freepik") {
      onChange({
        platform: id,
        titleLength: Math.min(settings.titleLength, 100),
        keywordsCount: 35,
      });
    } else {
        ...
        <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 space-y-6 transition-colors">
          ...
          <div>
            ...
            <input
              ...
              disabled={disabled || isFreepik}
              className="mt-2 w-full accent-slate-900 dark:accent-white disabled:opacity-50"
            />
            {isFreepik && (
              <p className="mt-1.5 text-[11px] text-slate-400">FreePik-এর জন্য কীওয়ার্ড সংখ্যা ৩৫-এ ফিক্সড</p>
            )}
          </div>
        </div>
      ) : (
